...
public class Agent2D : Agent
{
    ...
    {
        //print("observation collected..");
        // sensor.AddObservation(transform.localPosition);
        // sensor.AddObservation(goals[targetIndex].localPosition);
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        //distance reward
        float newDeltaX = DistanceToTarget();
        AddReward((deltaX - newDeltaX) * 0.2f);

        //Debug.Log("Distance reward: " + (deltaX - newDeltaX) * 0.2f);
        deltaX = newDeltaX;
        AddReward(-0.001f);
        //Debug.Log("Current cumulative reward: " + GetCumulativeReward());

        //collecting data
        RecordStats(uprightReward, newDeltaX);
       
    }

    void RecordStats(float uprightReward, float newDeltaX)
    {
        var recorder = Academy.Instance.StatsRecorder;
        ...
        // Log camera status every 500 steps
        if (logCameraInfo && agentCamera != null)
        {
            logCounter++;
            if (logCounter >= 500)
            {
                Debug.Log($"[CAMERA] Step {StepCount} - Camera active: {agentCamera.enabled}, Position: {agentCamera.transform.position:F2}");
                Debug.Log($"[CAMERA] Current targets in view: {goalsReached}/{goals.Length}, Distance to target: {newDeltaX:F2}");
                logCounter = 0;
            }
        }
    }
